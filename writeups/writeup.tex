...
Model 3 reveals, for instance, that a one unit increase in the urbanization level (the percentage of the total population living in an urban environment) is associated with a 3\% increase in GDP per capita (not \emph{log} GDP per capita; the distinction is important).
Similarly, a one percent increase in the urban population with access to sanitation is associated with a 3\% increase in GDP per capita. 
...
In model 5, we see that a 1\% increase in scientific journals and articles is associated with a 0.66\% increase in GDP per capita.
Similarly, a 1\% increase in the percentage of manufactured exports that are classified as ``high-tech'' is associated with a $2.1$\% increase in GDP per capita.
These are not surprising results.

However, we did find that a 1\% increase in research and development is associated with a $-0.75$ decrease in GDP per capita, which is surprising since one would expect R\&D to boost productivity and thus production.
...
According to model 3, a 1\% increase in male-infant life expectancy is, on average, expected to increase GDP per capita by $6.98$\%, and a 1\% increase in the refugee population is, on average, expected to decrease GDP per capita by $-0.13$\%. These are both unsurprising results. We expected richer countries to have higher life expectancy (better healthcare) and lower refugee populations (healthier political institutions).
