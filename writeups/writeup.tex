...
Figure~\ref{science_model_regressions} answers precisely that question. Indeed, there is no ``point'' -- in the sense of output and GDP -- to the science community if it doesn't produce anything. We know this because {\tt log.research:log.high.tech} and {\tt log.high.tech:log.journals} are significant, but {\tt log.research:log.journals} are not. % TODO interpret the interaction terms of lm(formula = log.GDP.per.capita ~ log.research + log.high.tech +    log.journals + log.research * log.high.tech + log.research *     log.journals + log.high.tech * log.journals)
